# Entry 3
##### 12/7/25
## Content
I brainstormed part b by searching up cooking robots/and other cooking technology. i finalized my content by ranking them from least complex to most complex. One of my idea was a cooking robot. As i noticed that not everyone in this world can cook every food or even know how to even cook so i wanted to make a robot that was able to cooking anything for you, i think for this technology to be invented in the next few year, maybe 20-30 years because the robots that we got right now are not perfect enough to cook without any mistakes image 1:(https://static.vecteezy.com/system/resources/previews/021/825/651/large_2x/robot-chef-cooking-in-kitchen-of-future-home-genius-smart-robot-working-in-modern-house-free-photo.jpg) image 2 (https://reviewed-com-res.cloudinary.com/image/fetch/s--6yMSrIui--/b_white,c_limit,cs_srgb,f_auto,fl_progressive.strip_profile,g_center,q_auto,w_972/https://reviewed-production.s3.amazonaws.com/1724189490000/Hero-20240816_MiniFridgeTBRNHeroV4_Renzi.PNG)



## EDP(Engineering Design Process)
For the EDP i'm currently on the foruth or fifth step as im trying to plan the best solution for my problem and also create a prototype of my solution.I'm on this step because i have no idea how i should plan for what my cooking robot would look like so i can move on to create my prototype.The next to my EDP is to test out my prototype to know if it works correctly and it doesn't have any bugs or glitches when i create it

## Skill
I have learned two skill which is:creativity and Growth mindset

## Creativity 
I have started to think more unique ideas for future projects.For example last month me and my partner came back with a idea to make a spinoff of a amine for my movie project. We made a orginal design that matched the color theme really well

## Growth mindset
Growth mindset changed the way i think as during the movie webpage project me and my partner didn't know how to do the linear griadent background so we decided to ask my teacher Mr.mueller for help on how to do it after he told 






[Previous](entry02.md) | [Next](entry04.md)

[Home](../README.md)
